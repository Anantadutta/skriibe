...
const Affiliators = () => {
  ...
  const fetchAffiliators = async () => {
    try {
      const res = await axios.get(`${import.meta.env.VITE_API_URL || 'http://localhost:5000/api'}/admin/affiliators`, { withCredentials: true });
      if (res.data.success) {
  ...
  return (
    <div style={{ padding: '40px', maxWidth: '1200px', width: '100%' }}>
      <h1 style={{ fontSize: '2rem', fontWeight: 800, marginBottom: '24px', color: '#fff' }}>Affiliate Network</h1>
